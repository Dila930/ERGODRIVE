...
export class WaktuReaksiKemudiPage implements OnInit, OnDestroy {
  ...
  trainingRecommendations = [
    {
      ...
      description: 'Latih kemudi dengan simulator untuk meningkatkan kecepatan reaksi',
  ...
  calculateSteeringScore(reactionTime: number, precision: number, stability: number): number {
    const reactionScore = Math.max(0, 100 - (reactionTime * 50));
    const weightedScore = (reactionScore * 0.4) + (precision * 0.35) + (stability * 0.25);
